...
The button injects into the VideoJS control bar and retries every 500ms for up to 30 seconds after the player loads. If it still doesn't appear, open browser DevTools (F12) and check the Console for errors prefixed with `[frame-capture-performer]`.

**GraphQL errors**
Check the browser console for the full error message. The most common cause is the video not being fully loaded — wait for playback to start and try again.
